...
@section('content')
    ...
    <div class="toolbar">
        ...
        <div class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <!-- begin :: Title -->
                <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1"><a href="{{ route('dashboard.stocks.index') }}"
                    class="text-muted text-hover-primary">{{ __("Stocks") }}</a></h1>
                <!-- end :: Title -->
    ...
    <div class="card">
        ...
        <div class="card-body p-0">
            <!-- begin :: Form -->
            <form action="{{ route('dashboard.stocks.update',$stock->id) }}" class="form submitted-form" method="post"  data-redirection-url="{{ route('dashboard.stocks.index') }}">
                @csrf
                @method('PUT')

                <!-- begin :: Card header -->
                <div class="card-header d-flex align-items-center">
                    <h3 class="fw-bolder text-dark">{{ __("Edit stock data") . " : #" . $stock->id  }}</h3>
                </div>
                <!-- end :: Card header -->
                ...
                <div class="inputs-wrapper">
                    ...
                    <div class="row mb-8">
                        ...
                        <div class="col-md-3 fv-row">
                            ...
                            <div class="form-floating">
                                <input required type="number" min="1" class="form-control" id="price_inp" name="price" value="{{ $stock->price }}" placeholder="example"/>
                        ...
                        <div class="col-md-3 fv-row">
                            ...
                            <div class="form-floating">
                                <input required type="number" min="1" class="form-control" id="delivery_price_inp" name="delivery_price" value="{{ $stock->delivery_price }}" placeholder="example"/>
                        ...
                        <div class="col-md-3 fv-row">
                            ...
                            <div class="form-floating">
                                <input type="text" class="form-control datepicker" id="delivery_date_inp" name="delivery_date" value="{{ $stock->delivery_date }}" placeholder="example"/>
                        ...
                        <div class="col-md-3 fv-row">
                            ...
                            <select required class="form-select" data-control="select2" name="status" id="status_inp" data-placeholder="{{ __("Select status") }}" data-dir="{{ isArabic() ? 'rtl' : 'ltr' }}" >
                                ...
                                @foreach (App\Enums\StockStatus::values() as $key => $value)
                                    <option value="{{ $value }}" {{ $stock->status == $value ? 'selected' : '' }}>{{ __(ucfirst(str_replace('_', ' ', $key))) }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-8 justify-content-center" id="products-container">
                        @foreach ($stock->products as $stockProduct)
                            <div class="border-secondary border rounded w-75 px-4 {{ $loop->index != 0 ? 'mt-5' : '' }}" id="product-{{ $loop->iteration }}">
                                ...
                                <div class="row my-5 align-items-center">
                                    ...
                                    <div class="col-md-6 fv-row">
                                        ...
                                        <select required class="form-select" data-control="select2" name="products[{{ $loop->index }}][id]" id="products_{{ $loop->index }}_id_inp" data-placeholder="{{ __("Select product") }}" data-dir="{{ isArabic() ? 'rtl' : 'ltr' }}" >
                                            ...
                                            @foreach (App\Models\Product::get() as $product)
                                                <option value="{{ $product->id }}" {{ $stockProduct->id == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                                            @endforeach
                                        </select>
                                        <p class="invalid-feedback" id="products_{{ $loop->index }}_id" ></p>
                                    </div>
                                    <!-- end :: Column -->
                                    ...
                                    <div class="col-md-5 fv-row">
                                        ...
                                        <div class="form-floating">
                                            <input type="text" class="form-control quantity-inp" id="products_{{ $loop->index }}_quantity_inp" data-index="{{ $loop->index }}" name="products[{{ $loop->index }}][quantity]" value="{{ $stockProduct->pivot->quantity }}" placeholder="example"/>
...
@push('scripts')
    <script>
        $(document).ready(function () {
            initDatePicker('delivery_date_inp', '{{ $stock->delivery_date }}');
        });
